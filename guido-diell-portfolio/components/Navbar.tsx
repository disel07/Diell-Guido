...
import { Menu, X, Code, ChevronDown, ExternalLink } from 'lucide-react';
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="relative">
              <motion.button
                ...
                onBlur={() => setTimeout(() => setProjectsOpen(false), 200)}
                className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-full font-mono text-sm text-cyber-primary transition-all"
              >
                PROJECTS <ChevronDown className={`w-4 h-4 transition-transform ${projectsOpen ? 'rotate-180' : ''}`} />
              </motion.button>
              ...
              <AnimatePresence>
                {projectsOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-64 bg-black/95 backdrop-blur-xl border border-cyber-primary/30 rounded-xl overflow-hidden shadow-[0_0_20px_rgba(0,243,255,0.1)] z-50"
                  >
                    <div className="py-2">
                      <div className="px-4 py-2 text-xs font-mono text-gray-500 border-b border-white/10 mb-1">
                        AI & PERSONAL LABS
                      </div>
                      {PROJECTS.map((project) => (
                        <a
                          ...
                          className="block px-4 py-3 hover:bg-cyber-primary/10 transition-colors group border-l-2 border-transparent hover:border-cyber-primary"
                        >
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-bold text-white group-hover:text-cyber-primary">{project.name}</span>
                            <ExternalLink className="w-3 h-3 text-gray-500 group-hover:text-cyber-primary" />
                          </div>
